...
> 어린이 지문 적성검사 사업의 **정식 홈페이지**. 리드 수집이 여전히 1순위 목표다.

> ⚠️ **2026-09-01 — 미학 방향 재수립 중.**
> 고객이 기존 "따뜻한 에디토리얼(Mastercard)" 방향을 기각하고, 더 **고급스러운** 느낌을 요청했다.
> 레퍼런스: `https://ablecare1.mycafe24.com/` (WordPress/Elementor, Swiper autoplay 캐러셀 히어로),
> 요청 형식: **"어라운드뷰" 히어로**.
> §2 Aesthetic Direction 은 `design-discovery` 로 교체 예정이다.
> §3 이후 토큰·스케일·모션·접근성은 **현재 코드가 실제로 쓰는 값**이므로 새 방향 승인 전까지 그대로 유효하다.
...
- **제품:** 학부모가 자녀의 지문 적성검사를 무료 상담으로 신청하는 정적 홈페이지 (구 1페이지 이벤트 랜딩)
- **사용자:** 자녀 1~다명을 둔 한국어 학부모 (모바일 비중 높음, 카카오·QR 유입 가정)
- **공간/카테고리:** 교육 리드 수집 — 의료적 진단이 아닌 “이해/강점/가능성”의 언어
- **제품 타입:** 정식 마케팅 사이트 — 상단 내비게이션 포함 (CTA 도착 후 폼 제출이 여전히 골든 패스)
- **프레임워크:** 의존성 없는 정적 HTML/CSS/JS
- **브랜드:** 너바나(NEOBANA) — 슬로건 “너를 알고, 나를 알다.” 로고 팔레트는 라벤더 퍼플 + 살구 오렌지 + 딥 네이비 + 아이보리. 현재 §3 팔레트(크림+시그널 오렌지)와 불일치하며, 새 방향은 로고에서 출발한다. 원본은 `data/` (gitignore).
...
**상태: 기각됨 — 교체 대기.** 아래는 2026-05-17 승인본의 기록이며, 새 방향 승인 시 이 절 전체를 대체한다.
...
| 2026-09-01 | 이벤트 랜딩 → 정식 홈페이지 전환 | 고객 요청. 상단 내비 허용, 섹션 구조 재정의 예정 |
| 2026-09-01 | 브랜드 자료 확인 (너바나 로고·슬로건) | 로고 팔레트(퍼플/살구/네이비)가 현행 크림+오렌지와 불일치 — 새 방향의 출발점 |
| 2026-09-01 | 브랜드명 「너바나 NEOBANA」 확정 | 로고·사업자등록증과 일치. “지문아이” 표기 폐기 |
| 2026-09-01 | 히어로 = 스크롤 워크스루(어라운드뷰 C안) | 3종 비교 시안에서 고객 선택 |
| 2026-09-01 | IA 5탭 + 고정 CTA, 단일 페이지 앵커 | 경쟁사 7~8탭 대비 축소. B2B는 푸터로 분리 |
| 2026-09-01 | IA 개편: 전문가 소개·MBTI·TCI·후기 추가, 결과 예시·검사 안내 삭제 | 목업 리뷰 후 고객 지시 |
| 2026-09-01 | 히어로 카피 “아이” → “너와 나” | 고객 지시. 슬로건 “너를 알고, 나를 알다”와 정렬. 매니페스토의 승인 카피 원문은 유지 |
| 2026-09-01 | Mastercard 미학 방향 기각 | 고객이 "고급진 느낌" 요청 (ablecare1.mycafe24.com 레퍼런스, 어라운드뷰 히어로). §2 재수립 대기 |
