...
> ⚠️ **Apps Script 재배포 필요 (2건 누적).** 관심 검사 필드 + 후기 기능이 추가되면서 `apps-script/Code.gs` 의
...
### Approved information architecture (2026-09-08 개편)

**멀티 페이지.** 메인 내비는 5개다 — 순서 고정: 전문가 소개 / 유전자지문적성검사 / TCI검사 / MBTI검사 / 후기 (+ 상담 신청 버튼).

| 파일 | 내용 |
|---|---|
| `index.html` | 워크스루 히어로 + 슬로건 + 우리의 약속 + 검사 3종 카드 + 하단 CTA |
| `expert.html` | 전문상담사 3인 프로필 (김의겸·김현아·이현아, 사진 `assets/people/`) |
| `dna.html` | 유전자지문적성검사 — 나침반 카피, 네 가지 시선, 검사 안내 79,000원 |
| `tci.html` | TCI 기질 및 성격검사 — 7척도, 부모·자녀 섹션, 검사 안내 59,000원 |
| `mbti.html` | MBTI 성격유형검사 — 4선호지표, 16유형, 관계 섹션, 검사 안내 49,000원 |
| `reviews.html` | **방문자가 직접 후기 작성** (별점·검사·내용) + 승인된 후기 목록 |
| `apply.html` | 리드 폼 + 관심 검사 선택 + 약관 모달 |
| `about.html`, `faq.html` | **메인 내비에서 제거(2026-09-08), 푸터 링크로만 유지.** SEO 자산이라 삭제하지 않았다 |

- **후기 게시 흐름:** 폼 제출 → `후기` 시트에 `승인=N` 으로 적립 + 알림 메일 →
  운영자가 시트에서 승인 열을 `Y` 로 바꾸면 `reviews.html` 이 `doGet?type=reviews` 로 불러와 게시.
  연락처는 절대 응답에 포함되지 않는다. 승인 전 후기는 사이트에 노출되지 않는다.
...
유전자지문적성검사 **79,000원** / TCI **59,000원** / MBTI **49,000원**.
각 페이지의 검사 안내 표와 `Service` 스키마 `Offer`(KRW)에 함께 들어가 있다.
